...
namespace afc
{
	namespace crc64Reversed_impl
	{
		 ...
		 */
		constexpr std::uint_fast64_t polynome = 0xc96c5795d7870f42;
		...
		// Indicates if aligned-8 data processing can be implemented in this platform.
		constexpr bool suitableForFastAligned8()
		{
			return afc::PLATFORM_BYTE_ORDER == afc::endianness::LE &&
					sizeof(std::uint_fast64_t) == 8 && CHAR_BIT == 8;
		}

		// Indicates if aligned-8 data can be exctracted from an arbitrary unsigned char data chunk.
		constexpr bool suitableForAutoFastAligned8()
		{
	...
	std::uint_fast64_t crc64ReversedUpdate_Aligned8Impl(std::uint_fast64_t currentCrc, const unsigned char *data,
			std::size_t n);

	inline std::uint_fast64_t crc64ReversedUpdate_Aligned8(const std::uint_fast64_t currentCrc,
			const unsigned char *data, const std::size_t n) {
		if (afc::crc64Reversed_impl::suitableForFastAligned8()) {
			// An optimised version that works only for little-endian platforms with 64-bit std::uint_fast64_t.
			return crc64ReversedUpdate_Aligned8Impl(currentCrc, data, n);
		} else {
			return crc64ReversedUpdate(currentCrc, data, n);
		}
	}

	// Each data chunk must be aligned by 4 octets. The result will be incorrect otherwise!
	std::uint_fast64_t crc64ReversedUpdate_Fast32(std::uint_fast64_t currentCrc,
			const unsigned char *data, std::size_t n);

	// Each data chunk must be aligned by 8 octets. The result will be incorrect otherwise!
	std::uint_fast64_t crc64ReversedUpdate_Fast64(std::uint_fast64_t currentCrc,
			const unsigned char *data, std::size_t n);

	// CRC-64 ECMA with LSB-first bit order (reversed).
	inline std::uint_fast64_t crc64Reversed(const unsigned char * const data, const std::size_t n)
	{
		return crc64ReversedUpdate(0, data, n);
	}

	/* CRC-64 ECMA with LSB-first bit order (reversed) with native platform byte endianness.
	 *
	 * It works on data aligned to 8-byte boundary only. On little-endian platforms this can
	 * gain some performance boost.
	 */
	inline std::uint_fast64_t crc64Reversed_Aligned8(const unsigned char * const data, const std::size_t n)
	{
		return crc64ReversedUpdate_Aligned8(0, data, n);
	}

	// CRC-64-ECMA with LSB-first bit order (reversed) with native platform byte endianness.
	template<typename Iterator>
	std::uint_fast64_t crc64ReversedUpdate(const std::uint_fast64_t currentCrc, Iterator begin, Iterator end)
	{
	...
	// CRC-64-ECMA with LSB-first bit order (reversed) with native platform byte endianness.
	template<typename Iterator>
	std::uint_fast64_t crc64Reversed(Iterator begin, Iterator end) { return crc64ReversedUpdate(0, begin, end); }
}
